...
Clarity MCPサーバーが利用可能。CPA分析やLP行動分析時に**リアルタイムデータを取得**できる。

| ツール | 用途 |
|--------|------|
| `query-analytics-data` | LP上のユーザー行動データ（スクロール深度・クリック率・離脱率等） |
| `list-session-recordings` | セッション録画一覧（デバイス・ブラウザ・エラー有無等でフィルタ可） |

活用タイミング:
- CPA上昇の原因分析 → 広告データ＋Clarityのユーザー行動を突き合わせる
- クリエイティブ変更後 → LPでの実際の行動変化をClarityで検証
- ファネルボトルネック特定 → LP上のどこでユーザーが離脱しているかを数値で確認

## KPIダッシュボード（広告分析の正式データソース）

広告分析は**KPIダッシュボード**スプレッドシートを正式なデータソースとして使用する。

- **現行**: [【オートウェビナー】広告分析ダッシュボード](https://docs.google.com/spreadsheets/d/1lq2N5KtvdNaFqZweEaN2n3nAoFuHRrWKUU8TIseZN_M/)
- **構成テンプレート**: メモリ `reference_kpi_dashboard.md` に列構成・数式・書式の完全仕様を記録済み
- **新キャンペーン作成時**: テンプレートに従い同構成のスプレッドシートを複製・作成する

### ダッシュボードの列順（ファネル順）
広告費 → IMP → クリック → CTR → CPC → LINE登録 → LINE追加率 → **CPA** → 説明会 → 個別相談 → 成約数 → **CPO** → 面談終了 → 面談実施率 → キャンセル → 成約金額 → ROAS

### 指標の定義（重要）
- **CPA** = 広告費 ÷ LINE登録数（1リスト獲得コスト）
- **CPO** = 広告費 ÷ 成約数（1成約獲得コスト）
- CPA ≠ CPO。混同しないこと
...
- `knowledge/marketing/auto-webinar/analytics/clarity.md` — Clarity行動分析データ（過去の静的データ。最新はMCPツールで取得）
