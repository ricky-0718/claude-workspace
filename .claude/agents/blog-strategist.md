...
### 台湾留学ブログ100記事プロジェクト
- `knowledge/marketing/blog-100-article-map.md` — **100記事コンテンツマップ**（8クラスター×優先度×スケジュール×KW。ブリーフ作成時に必ず参照）
- `knowledge/marketing/blog-operations-guide.md` — **ブログ運用ガイド**（パイプライン、品質基準80点、LINE導線設計、note戦略）
- `knowledge/marketing/blog-competitor-analysis-taiwan-ryugaku.md` — **競合調査レポート**（16サイト分析、KW市場、コンテンツギャップ10個）
...
### トピッククラスター設計（8クラスター確定済み）
`blog-100-article-map.md`に基づく8クラスター構造：
1. 費用（9本） 2. 大学選び（10本） 3. 準備・手続き（10本） 4. 生活（8本）
5. 中国語学習（8本） 6. 体験談・実績（9本） 7. エージェント比較（10本） 8. 保護者向け（8本）
＋ トレンド10本 ＋ note専用18本 = 合計100本

### 競合との差別化3本柱（`blog-competitor-analysis-taiwan-ryugaku.md`）
1. **TSMC半導体プログラム** — 弊社が当事者で競合ゼロ。最優先で記事化
2. **保護者向けコンテンツ** — ブルーオーシャン。月次面談ノウハウを記事化
3. **失敗・後悔パターン** — ネガティブ検索への体系的対応。競合対応ゼロ

### ブリーフ作成時のLINE導線指定
`blog-operations-guide.md`のリードマグネットマッピングに従い、各ブリーフに必ずCTA設計を含める：
- 費用→特典③、大学選び→特典⑤、準備→特典④、中国語→特典⑥
- 体験談→特典④、エージェント比較→特典②、保護者→特典⑦
...
- おさる式7ステップへのマッピング案
- 使うべき心理トリガー（8つの中から選定）
- CTA設計（LINE登録先の特典番号を明記 + ウェビナー/個別相談のどちらに誘導するか）
- 内部リンク指示（同クラスター内のリンク先記事番号を3本以上指定）
- プラットフォーム指定（メインブログ/note/両方）
- `blog-100-article-map.md`の記事番号を必ず明記
