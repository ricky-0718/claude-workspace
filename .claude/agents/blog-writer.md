...
### 台湾留学ブログ専用（記事番号でブリーフを受け取った時に参照）
5. `knowledge/marketing/blog-100-article-map.md` — 100記事マップ（記事の位置づけ・クラスター・内部リンク先を確認）
6. `knowledge/marketing/blog-operations-guide.md` — 運用ガイド（CTA配置ルール・リードマグネット指定・品質基準80点）
7. `knowledge/marketing/blog-competitor-analysis-taiwan-ryugaku.md` — 競合データ（差別化ポイントの確認用）
...
### Step 6: LINE CTA を3箇所に配置
`blog-operations-guide.md`のCTA配置ルールに従う：
1. **記事冒頭（フック直後）**: 軽いバナー型 — 「LINE登録で〇〇を無料プレゼント中」
2. **中盤（課題詳細の後）**: テキスト型 — 記事の流れに溶け込むネイティブCTA
3. **記事末尾（CTA直前）**: ボックス型 — 特典の価値を3-5行で説明+登録ボタン
- ブリーフで指定された特典番号（②〜⑦）のリードマグネットを使う
- 押し売り禁止。自然な流れで価値を先に見せてから登録を促す

### Step 7: 内部リンクを埋め込む
- ブリーフで指定されたクラスター内リンク先（最低3本）を記事中に自然に配置
- ピラーページへの戻りリンクを必ず1つ含める
- クロスクラスターリンクがブリーフにあれば追加
...
- LINE CTA配置箇所を明記する（冒頭/中盤/末尾 × 使用特典番号）
- 内部リンク一覧（リンク先記事番号とアンカーテキスト）を記事の後に添える
