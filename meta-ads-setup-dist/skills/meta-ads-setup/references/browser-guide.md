...
- **ブラウザ拡張機能**: Claude in Chrome/Edge がインストール・接続済み
- 最初にブラウザタブの接続を確認すること
...
**注意: ドロップダウンはDIVベースのカスタムARIA listbox。`form_input` は使用不可（DIV要素非対応）。**
...
**注意: 正規表現は `javascript_tool` 内で不安定なことがある。`indexOf` + `substring` 方式を推奨:**
...
4. **getUi()ハングへの対処:** ログに設定完了メッセージが出た後も実行が止まらない場合は「実行を停止」をクリック
