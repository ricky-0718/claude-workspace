...
Meta広告アカウント内の新しいキャンペーンに対して、
**スプレッドシート作成 → GASコード設定 → API認証設定 → トリガー設定** を一括で行う。

## 前提条件

- ブラウザ拡張機能（Claude in Chrome/Edge）が接続済み
- Meta広告のAPIアクセス権限があること（ビジネスマネージャのシステムユーザー or Graph API Explorerのユーザートークン）
...
4. **（既存の場合）スプレッドシートURL**
5. **Meta API認証情報**（初回セットアップ時のみ。2回目以降は既存プロジェクトから再利用可能）:
   - App ID
   - App Secret
   - 広告アカウントID（`act_` で始まる）
   - アクセストークン（システムユーザートークン推奨。有効期限60日）
   - エラー通知先メールアドレス
...
| `{{AD_ACCOUNT_ID}}` | ユーザーの広告アカウントID |
| `{{TRIGGER_HOUR}}` | 既存トリガーとずらした時刻（デフォルト: 4時） |
...
function _tempSetCredentials() {
  ...
  props.setProperties({
    'META_APP_ID': '{ユーザーのApp ID}',
    'META_APP_SECRET': '{ユーザーのApp Secret}',
    'META_ACCESS_TOKEN': '{ユーザーのアクセストークン}',
    'META_AD_ACCOUNT_ID': '{ユーザーの広告アカウントID}',
...
**注意:**
- システムユーザートークン推奨（60日有効、ビジネスマネージャで生成）
- ユーザートークンの場合は `exchangeForLongLivedToken()` で長期トークンに変換
- 一時関数実行後は必ず削除してセキュリティを確保

**2回目以降のセットアップ（既存プロジェクトからトークン再利用）:**
1. 既存の稼働中GASプロジェクトのApps Scriptエディタを開く
