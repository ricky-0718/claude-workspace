...
オーナーの業務を支援する10体の専門エージェントが `.claude/agents/` に配置されている。
...
| **student-advisor**（出願アドバイザー） | 出願サポート・志望校選定・書類準備・学生データ管理 | 出願、大学選び、TOCFL、書類準備、合格実績、学生チャットボットの話題 |
...
### フォーム設定の「他シナリオ連携」禁止
- シナリオの「登録フォーム・読者項目」で**「他シナリオ連携」が「する」になっている項目は絶対に変更・削除しない**
- 全シナリオと同期している共有設定。1箇所の変更が全シナリオに波及し、復旧困難
- ステップ配信の追加・削除はOK（フォーム設定とは別）
- シナリオ設定（名前・グループ等）の変更もOK
- ブラウザ自動化時、フォーム設定ページでの操作は特に慎重に（誤クリック防止）
