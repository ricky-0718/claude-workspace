...
## Git Worktree ルール
- **セッション終了時**: ワークツリーは必ず `git worktree remove <パス>` で削除する（`rm -rf` 禁止）
- **作業完了時**: masterにマージ → ブランチ(`git branch -D`)＆ワークツリー両方を削除
...
- **セクション**: 「新良理輝」セクション
- **記入方法**:
  1. 完了したタスクごとに、タスク名（`M/DD タスク名` 形式）を生成してユーザーに提示する
  2. ユーザーがAsanaでタスクを作成し、リンクを共有する
  3. `update_task` でタスク名・説明文・担当者を自動入力する
- **タスク説明のフォーマット**:
  ```
  ■ やったこと
  [何を改善・完了したかの説明]
